...
assorbimento_df <- rast_df[grepl("assorbimento", names(rast_df))] #estraggo solo i dataframe della resistanza

fun_plot_assorbance <- function(x) ggplot(x,aes(x=x,y=y))+ #funzione per l'heatmap
  stat_contour_filled(aes(z=value,fill='green'),bins = 1,show.legend = F)+
  labs(x = NULL, y = NULL) + #tolgo i label agli assi
  coord_fixed() #per avere un quadrato sempre
plot_assorbance <- lapply(assorbimento_df,fun_plot_assorbance)
...
plot_assorbance_norm <- mapply(plot=plot_assorbance,nomi=nomi_plot,fun_nomi_plot,SIMPLIFY = F) #ho due liste, una dei plot e una dei nomi, applico una funzione multipla, simplify=F serve a ricreare due liste, altrimenti per ogni layer dei plot usa la funzione
```

```{r heatmap dispersione, message=FALSE, include=FALSE}
dispersione_df <- rast_df[grepl("dispersione", names(rast_df))] #estraggo solo i dataframe della resistanza
...
plot_dispersion_norm <- mapply(plot=plot_dispersion,nomi=nomi_plot,fun_nomi_plot,SIMPLIFY = F) #ho due liste, una dei plot e una dei nomi, applico una funzione multipla, simplify=F serve a ricreare due liste, altrimenti per ogni layer dei plot usa la funzione
```

```{r heatmap mortalita, message=FALSE, include=FALSE}
mortalita_df <- rast_df[grepl("mortalita", names(rast_df))] #estraggo solo i dataframe della resistanza
...
plot_mortality_norm <- mapply(plot=plot_mortality,nomi=nomi_plot,fun_nomi_plot,SIMPLIFY = F) #ho due liste, una dei plot e una dei nomi, applico una funzione multipla, simplify=F serve a ricreare due liste, altrimenti per ogni layer dei plot usa la funzione
```
...
do.call(grid.arrange,c(plot_absorbance_norm, ncol = (length(raster_files)/(length(raster_dir)-1))))
```
...
do.call(grid.arrange,c(plot_dispersion_norm, ncol = (length(raster_files)/(length(raster_dir)-1))))
```
...
do.call(grid.arrange,c(plot_mortality_norm, ncol = (length(raster_files)/(length(raster_dir)-1))))
```
